...
latlon_from_mactsubpart <- function(subpart = "JJJ", include_if_no_latlon = FALSE) {
  ...
  if (!exists("frs_by_mact_arrow")) {
    dataload_dynamic("frs_by_mact", return_data_table = FALSE)
  }
  
  
  query <- subpart
  
  if (missing(subpart) || !is.atomic(query) || !is(query,"character")) {
  ...
  #   warning("subpart must be a single character string like 'AAAA' ")
    ...
    return(NA) #Might as well return early instead of looking for empty string
    }
  query <- toupper(query)
  mact_out <- frs_by_mact_arrow %>%
    filter(.data$subpart %in% !!query) %>%
    collect() %>% 
    data.table::as.data.table() # was using == which did not work for multiple codes in query.
