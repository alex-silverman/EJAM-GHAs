...
To include specific columns provides those as a character vector of varnames.")
      ...
      if ((is.null(sf_data.frame)) & !('radius.miles' %in% varnames)) {
        varnames = c(varnames, 'radius.miles')
        } # radius will be needed to draw circles, unless sf_data.frame provided
...
Except, if Counties were analyzed, see  mapfastej_counties() \n')
    ...
    if ('radius.miles' %in% names(df)) {
      ...
      warning('radius.miles is missing, using arbitrary value of 0 miles to create a shapefile of circles around points')
      radius <- 0
    }
