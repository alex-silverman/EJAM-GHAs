...
doaggregate <- function(sites2blocks, sites2states_or_latlon=NA,
  ...
  if (is.function(updateProgress)) {
    ...
    updateProgress(message_main = boldtext, value = as.numeric(difftime(Sys.time(), start_time, units = "secs"))) #value was 0.2 in non timer approach
  }
  ##################################################### #
  ...
  if (is.function(updateProgress)) {
    ...
    updateProgress(message_main = boldtext, value = as.numeric(difftime(Sys.time(), start_time, units = "secs"))) #Value was 0.4 in non timer approach
  }
  #***  ###################################### #
  #____________________________________________________   #  ##################################################### #  ######################################################
  ...
  if (need_proximityscore) {
    ...
    sites2bgs_bysite   <- sites2blocks[         , {    
      
      if (.GRP %% 50000 == 0 && is.function(updateProgress)) {
      boldtext <- paste0('Analyzing blockgroups')
      updateProgress(message_main = boldtext, 
                     value = as.numeric(difftime(Sys.time(), start_time, units = "secs")))
    }
      .(bgwt = sum(blockwt, na.rm = TRUE),     # 550 msec ??
    ...
    sites2bgs_overall  <- sites2blocks_overall[ , {
      ...
      .(bgwt = sum(blockwt, na.rm = TRUE),    # 318 msec
  ...
  #    avg of mins by Demog group: save by bg (or maybe by bg by site) to summarize avg by residential population group, overall and possibly by site.
  ...
  # ( ENVT, and if include_ejindexes=TRUE, the Summary Indexes too )
  ##################################################### #
  ...
  ### (dont need avg of raw Summary Indexes
  ## (we dont want to show a ratio to avg for raw Summary Indexes, and no other reason to find those averages, since those raw scores are harder to interpret than Residential Pop. or Envt raw scores)
  ...
  data.table::setnames(avg.cols_overall,  names_these,  names_these_avg)
  ...
  { # (can fold code here) ----------------------------------------\
    ...
    useful_column_order <- c(
      ...
      ## # Top of report  EXTRA residential population groups data
      ...
      ## # Bottom of report  A mix of population group - LIKE / ENVT-LIKE / EJ-LIKE / on report
    ...
    names_age_count,
      ...
      ## SUMMARY INDEXES (if include_ejindexes=TRUE) ----------------------------------------\
