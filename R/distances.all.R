...
distances.all <- function(frompoints, topoints, units='miles',
  ...
  if (!return.rownums & return.latlons) {
    ...
    results <- cbind(expand_gridMatrix(topoints[,'lat'], frompoints[,'lat']),expand_gridMatrix(topoints[,'lon'], frompoints[,'lon']) , as.vector( t(results.matrix) ) )
  ...
  if (return.rownums & !return.latlons) {
    ...
    results = cbind(expand_gridMatrix(1:length(topoints[,'lat']), 1:length(frompoints[,'lat'])), as.vector( t(results.matrix) )  )
  ...
  if (return.rownums & return.latlons) {
    ...
    results <- cbind(expand_gridMatrix(topoints[,'lat'], frompoints[,'lat']),expand_gridMatrix(topoints[,'lon'], frompoints[,'lon']) , results)
