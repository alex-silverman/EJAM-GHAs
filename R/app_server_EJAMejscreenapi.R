...
app_server_EJAMejscreenapi <- function(input, output, session) {
  ...
  output$mapout <- leaflet::renderLeaflet({
    ...
    if (length(mypoints) != 0) {
      ...
      mymap <- leaflet::leaflet() %>% leaflet::addTiles() %>% leaflet::setView(-110, 46, zoom = 3)
      mymap
    }
    ### Button to print map ####
  ...
  observe({ # this displays the proxy map that changes if radius or selected rows change, etc.
    ...
    if (length(mypoints) != 0) {
      ...
      mylivemap <- leaflet::leafletProxy("mapout", data = mypoints) %>% leaflet::clearShapes() %>%
        leaflet::addCircles(lat = ~latitude, lng = ~longitude,
      ...
      if (input$cluster_highlighting_on) {
        ...
        leaflet::addCircles(lng = some$longitude, lat = some$latitude, 
