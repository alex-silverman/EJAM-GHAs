...
app_server <- function(input, output, session) {
  ...
  observe({
    if (length(golem::get_golem_options("shiny.testmode")) > 0  ) { # allow params in run_app() to override default
      if (!golem::get_golem_options("shiny.testmode")) {
  ...
  if (!is.null(golem::get_golem_options('advanced'))) { # option provided to run_app()
  ...
  data_up_epa_program_up <- reactive({
    ...
    pgm_out <- pgm_out %>%
      latlon_df_clean(invalid_msg_table = T)
  ...
  data_up_mact <- reactive({
    ...
    if (all(is.na(mact_out$lat)) & all(is.na(mact_out$lon))) {
      ...
      invalid_alert[[  placetype]] <- mact_out[valid==F, .N]
      ## return output dataset
      mact_out
    }
  })
  ...
  observe({
    ...
    if(disable_buttons[[current_upload_method()]]){
  ...
  output$invalid_sites_alert2 <- renderUI({
    ...
    build_invalid_alert(
      invalid_count = invalid_alert[[current_upload_method()]],
      method = input$ss_choose_method
    )
  })
  ...
  observe({
    req(data_uploaded())
    an_map_text_pts[[current_upload_method()]] <- build_sitecount_text(current_upload_method(), data_uploaded())
  })
  ...
  output$an_map_text <- renderUI({
    ...
    HTML(an_map_text_pts[[current_upload_method()]])
  })
  ...
  observeEvent(input$bt_get_results, {  # (button is pressed)
    ...
    if (!(submitted_upload_method() %in% c('SHP', 'FIPS'))) {
      nrows = nrow(data_uploaded()[!is.na(lat) & !is.na(lon),])
      radius = sanitized_bt_rad_buff()
      predicted_runtime <- predict_ejamit_runtime(nrows, radius)
      if (predicted_runtime[,"fit"] > 120) { #Use the predicted time for ejammit, add warning in app if greater than 2 minutes
        ejamitRunTimeNotification <- showNotification(
          paste("Warning: Predicted runtime is", round(predicted_runtime[,"fit"], 0), 
                "seconds."),
          type = "message",
          duration = NULL
        )
    ...
    if (submitted_upload_method() == 'FIPS') {  # if FIPS, do everything in 1 step right here.
      ...
      nrows_blocks_value <- nrow(sites2blocks)
      ...
      updateProgress_doagg <- function(value = NULL, message_detail = NULL, message_main = NULL, upper_bound = upper_bound_value) {
        if (!is.null(value) && !is.null(upper_bound)) {
          progress_fraction <- min(1, value / upper_bound)
        } else if (is.null(value)) {
          current_value <- progress_doagg$getValue()
          progress_fraction <- current_value + (progress_doagg$getMax() - current_value) / 5
        } 
        
        progress_doagg$set(value = progress_fraction, message = message_main, detail = message_detail)
      }
      ...
      if (submitted_upload_method() != "FIPS") {  # if LAT LON or SHAPEFILE, now have blocks nearby and ready to aggregate
        ...
        if (submitted_upload_method() != 'SHP') {
          out$results_bysite[ , `:=`(
            ...
            `ACS Report`      = 'N/A',#url_ejscreen_acs_report(lat = d_upload$lat, lon =  d_upload$lon, radius = sanitized_bt_rad_buff(), as_html = TRUE),
          ...
          out$results_bysite <- out$results_bysite %>%
            dplyr::mutate(
              `EJScreen Report` = 'N/A',#ifelse(valid == T, url_ejscreen_report(    lat = d_upload$lat, lon =  d_upload$lon, radius = sanitized_bt_rad_buff(), as_html = TRUE), 'N/A'),
              `EJScreen Map`    = 'N/A',#ifelse(valid == T, url_ejscreenmap(        lat = d_upload$lat, lon =  d_upload$lon,                             as_html = TRUE),  'N/A'),
              `ACS Report`      = 'N/A',#url_ejscreen_acs_report(lat = d_upload$lat, lon =  d_upload$lon, radius = input$bt_rad_buff, as_html = TRUE),
    ...
    if (exists("ejamitRunTimeNotification") && !is.null(ejamitRunTimeNotification)) {
      removeNotification(ejamitRunTimeNotification)
    }
  ...
  ## *** This code should be harmonized with and/or replaced by function like ejam2report()
  ...
  observeEvent(
    ...
    eventExpr = {
      ...
      title_text <- build_summary_title(analysis_title = sanitized_analysis_title(),
                                        upload_method = current_upload_method(),
                                        radius =  sanitized_bt_rad_buff(), 
                                        nsites = NROW(data_processed()$results_bysite),
                                        pop = total_pop())
  ...
  report_map <- reactive({
    ...
    ## *** This code should be harmonized with and/or replaced by function like mapfast() 
    ...
    if (submitted_upload_method() == "SHP") {
      ...
      if (!is.na(rad_buff) && rad_buff > 0) {
        ...
        leaflet::leaflet(d_uploads, width=if(isTRUE(getOption("shiny.testmode"))) 1000 else NULL) %>%  
          leaflet::addTiles()  %>%
          leaflet::addPolygons(data = d_uploads, color = circle_color,
        ...
        leaflet::leaflet(data_spatial_convert, width=if(isTRUE(getOption("shiny.testmode"))) 1000 else NULL) %>% 
          leaflet::addTiles()  %>%
      ...
      if (submitted_upload_method() != "FIPS") {
        ...
        leaflet::leaflet(data_processed()$results_bysite, width=if(isTRUE(getOption("shiny.testmode"))) 1000 else NULL) %>% #,
          ...
          leaflet::addCircles(
            ...
            if (all(FTYPES %in% c("state", "county", "city", "tract", "blockgroup"))) {
              ...
              fips_shapes <- shapes_from_fips(data_uploaded())
              
              if (!is.null(fips_shapes) && any(!sf::st_is_valid(fips_shapes))) {
                fips_shapes <- sf::st_make_valid(fips_shapes)
              }
              if (!is.null(fips_shapes) && nrow(fips_shapes) > 0) {
                
                leaflet::leaflet(fips_shapes) %>%
                  leaflet::addTiles() %>%
                  leaflet::addPolygons(
                    data = fips_shapes,
                    color = "green",
              ...
              leaflet::leaflet(width=if(isTRUE(getOption("shiny.testmode"))) 1000 else NULL) %>% 
                leaflet::addTiles() %>% 
                leaflet::fitBounds(-115, 37, -65, 48)
            }
    ...
    if (current_upload_method() == "SHP") {
      if (!is.na(rad_buff) && rad_buff > 0) {
        ...
        leaflet::leafletProxy(mapId = 'an_leaf_map', session) %>%
          leaflet::addPolygons(data = d_uploads, color = "red")
      }
      d_uploads <- data_uploaded() %>%
        dplyr::select(-any_of(c('valid', 'invalid_msg'))) %>%
        sf::st_zm() %>% sf::as_Spatial() # st_zm() was already done?
      leaflet::leafletProxy(mapId = 'an_leaf_map', session) %>%
        leaflet::addPolygons(data = d_uploads,
      ...
      if (current_upload_method() == 'FIPS') {
        ...
        leaflet::leafletProxy(mapId = 'an_leaf_map', session) %>%
          map_shapes_leaflet_proxy(shapes = shapes_from_fips(fips = data_uploaded()))
  ...
  v1_summary_plot_state <- reactive({
    ...
    if (!is.null(cur_button())) {
      ...
      ejam2barplot(
        ...
        main = "Residential Populations at the Analyzed Location \n Compared to State Averages"
      )
    } else {
  ...
  v1_summary_plot <- reactive({
    ...
    if (input$plotkind_1pager == 'bar') {
      ...
      req(data_processed())
      rationames = c(names_d_ratio_to_avg, names_d_subgroups_ratio_to_avg)
  ...
  output$view3_table <- DT::renderDT(server = TRUE, expr = {
    req(data_processed())
    create_view3_table(data_processed = data_processed(),
