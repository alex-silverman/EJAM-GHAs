...
report_setup_temp_files <- function(Rmd_name = 'community_report_template.Rmd', Rmd_folder = 'report/community_report/') {
  ...
  if (!file.exists(file.path(tempdir( ), 'communityreport.css'))) {
    file.copy(from = app_sys(paste0(Rmd_folder, 'communityreport.css')),
  ...
  if (!file.exists(file.path(tempdir( ), 'www', 'EPA_logo_white_2.png'))) {
    ...
    file.copy(from = app_sys(paste0(Rmd_folder, 'EPA_logo_white_2.png')),
...
report_community_download <- function(file,
  ...
  if (!is.null(row_index)) {
    ...
    ## While ejscreen site is down and cannot provide the normal full summary report on a single site,
    ## disable barplot version and instead 
    ## do full summary version for single site (just like done for the summary of sites).
    ##  
