...
#'   EJScreen has not had residential population data in those locations anyway, but can map sites there.
...
latlon_is.valid <- function(lat, lon, quiet = TRUE, invalid_msg_table = FALSE , exact_but_slow_islandareas = FALSE) {
  ...
  if (any(in_islandareas, na.rm = TRUE)) {
    message("Some points appear to be in US Island Areas, which may lack some data such as residential population data here")
  }
