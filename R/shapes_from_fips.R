...
shapes_from_fips <- function(fips) {
  ...
  if (length(intersect(ftype, types))>1){
      stop("This dataset contains more than one type of FIPS code. Analysis can only be ran on datasets with one type of FIPS codes.")
  }
...
#'
#' @param fips vector of 7-digit City/town/CDP codes as in \code{censusplaces$fips}
#' @seealso [shapes_from_fips()]
