...
# all.equal(testinput_registry_id, EJAM::testinput_registry_id)
all.equal(testinput_program_sys_id, EJAM::testinput_program_sys_id)

## use_data() ####

usethis::use_data(testinput_program_sys_id, overwrite = TRUE)
...
x <- frs_from_programid(testinput_program_sys_id$program, testinput_program_sys_id$pgm_sys_id)[, c("PGM_SYS_ACRNMS", "PRIMARY_NAME")]
savex(x, "./inst/testdata/programid",  "testinput_program_sys_id_8.xlsx")
rm(x)
rm(savex)
#################################### #
cat('
