...
testthat::test_that("fixcolnames_infer works at all", {
  ...
  testinput_address_table_renamed <- testinput_address_table
  testthat::expect_no_error({names(testinput_address_table_renamed) <- fixcolnames_infer(names(testinput_address_table_renamed))})
  testthat::expect_identical(testinput_address_table_goodnames, testinput_address_table_renamed)
  
  testthat::expect_equal(fixcolnames_infer(currentnames = testinput_address_parts1),
                             c("lat", "lon", "address", "street", "city", "state", "zip"),
                            ignore_attr = TRUE)
  testthat::expect_identical(fixcolnames_infer(currentnames = names(testinput_address_table) ),
...
run_renaming_tests = function(tlist, oldnames, testname="test", fun2test = fixcolnames_infer) {
  ...
  if (identical(fixcolnames_infer,  fun2test)  | identical(fun2test , latlon_infer))  {
    if (identical(fun2test , latlon_infer)) {
